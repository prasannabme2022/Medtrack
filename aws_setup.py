...
from ai_engine import (
    upload_image_to_s3, analyze_image_rekognition, extract_medical_entities,
    generate_bedrock_prediction, generate_mock_prediction,
    save_prediction, get_patient_predictions, get_prediction_by_key,
    get_high_critical_predictions, send_ai_alert, run_ai_pipeline
)
...
try:
    ...
    try:
        ...
        logger.info("AI service clients (S3, Rekognition, ComprehendMedical) initialized")
    except Exception as ai_e:
        logger.warning(f"AI service clients not available: {ai_e}")
        s3_client = None
    ...
    TABLES_CONFIG = [
        ...
        {'name': PREDICTIONS_TABLE, 'key': 'patient_id', 'sort_key': 'timestamp'}
    ]
        ...
        for table_config in TABLES_CONFIG:
            ...
            if table_name not in existing_tables:
                ...
                try:
                    ...
                    attr_defs = [{'AttributeName': key_name, 'AttributeType': 'S'}]
    ...
    predictions_table = LocalStorage(PREDICTIONS_TABLE)
...
def get_patient(email):
    """Get patient by email from DynamoDB, with local fallback"""
    try:
        response = patients_table.get_item(Key={'email': email})
        return deserialize_item(response.get('Item'))
    except Exception as e:
        logger.warning('DynamoDB unavailable for get_patient, using local fallback: ' + str(e))
        try:
            from local_storage import LocalStorage
            local = LocalStorage('medtrack_patients')
            resp2 = local.get_item(Key={'email': email})
            return deserialize_item(resp2.get('Item'))
        except Exception as e2:
            logger.error('Error getting patient: ' + str(e2))
            return None
...
def update_patient(email, updates):
    ...
    try:
        ...
        logger.warning("DynamoDB unavailable (\1), local fallback: " + str(e)[:80])
        return False
...
def get_doctor(email):
    """Get doctor by email from DynamoDB, with local fallback"""
    try:
        response = doctors_table.get_item(Key={'email': email})
        return deserialize_item(response.get('Item'))
    except Exception as e:
        logger.warning('DynamoDB unavailable for get_doctor, using local fallback: ' + str(e))
        try:
            from local_storage import LocalStorage
            local = LocalStorage('medtrack_doctors')
            resp2 = local.get_item(Key={'email': email})
            return deserialize_item(resp2.get('Item'))
        except Exception as e2:
            logger.error('Error getting doctor: ' + str(e2))
            return None
...
# ============================================
# AI HEALTH COMPANION - PREDICTIVE ENGINE ROUTES
# ============================================
...
def ai_companion():
    ...
    return render_template('patient/ai_companion.html')

@app.route('/ai_predict', methods=['POST'])
def ai_predict():
    """Trigger the full AWS AI prediction pipeline"""
    if 'user_id' not in session or session.get('role') != 'patient':
    ...
    try:
        ...
        result = run_ai_pipeline(
            ...
            comprehend_medical_client=comprehend_medical_client,
            sns_client=sns_client if AWS_AVAILABLE else None,
            sns_topic_arn=SNS_TOPIC_ARN,
            predictions_table=predictions_table,
            ai_s3_bucket=AI_S3_BUCKET,
            ai_available=AI_SERVICES_AVAILABLE if AWS_AVAILABLE else False
        )
        ...
        if result and result.get('prediction'):
            ...
            if severity in ('High', 'Critical'):
                flash(f'AI Prediction Complete: {severity} severity detected. Your doctor has been alerted.', 'warning')
            else:
                flash('AI Prediction Complete! View your results below.', 'success')
            
            return redirect(url_for('ai_results'))
        else:
            flash('AI prediction pipeline failed. Please try again.', 'error')
            return redirect(url_for('ai_companion'))
            
    except Exception as e:
        logger.error(f"AI prediction error: {e}")
        flash(f'An error occurred during AI analysis. Please try again.', 'error')
        return redirect(url_for('ai_companion'))

@app.route('/ai_results')
def ai_results():
    """Display the structured prediction report"""
    if 'user_id' not in session or session.get('role') != 'patient':
        flash('Please login as a patient first.', 'info')
        return redirect(url_for('login'))
    ...
    # Check for specific prediction via query params
    req_patient = request.args.get('patient_id', patient_id)
    req_timestamp = request.args.get('timestamp', '')
    
    prediction = None
    
    if req_timestamp:
        # Load specific prediction from DynamoDB
        prediction = get_prediction_by_key(req_patient, req_timestamp, predictions_table)
    
    if not prediction:
        # Try session stored prediction
    ...
    if not prediction:
        flash('No prediction results found. Please run an AI analysis first.', 'info')
        return redirect(url_for('ai_companion'))
    ...
    return render_template('patient/ai_results.html', 
...
def doctor_ai_alerts():
    """Doctor-facing page showing AI-flagged High/Critical cases"""
    if 'user_id' not in session or session.get('role') != 'doctor':
        flash('Please login as a doctor first.', 'info')
        return redirect(url_for('login'))
    
    # Get all high/critical predictions
    alerts = get_high_critical_predictions(predictions_table)
    
    # Enrich with patient names
    for alert in alerts:
        pid = alert.get('patient_id', '')
        patient = get_patient(pid)
        alert['patient_name'] = patient.get('name', pid) if patient else pid
    ...
    return render_template('doctor/doctor_ai_alerts.html',
                         alerts=alerts,
